  ...
  apply(compiler) {
    ...
    attachAfterEmitHook(compiler, (compilation, cb) => {
      this.finalizeRelease(compilation).then(() => {
        if (typeof this.options.cleanUp === "function") {
          return this.options.cleanUp();
        }
        if (typeof this.options.cleanUp === "string" && fs.existsSync(this.options.cleanUp)) {
          return require(path.resolve(this.options.cleanUp))(this.options, compilation);
        }
      }).then(() => cb());
    });
  }
  replaceContent(compilation) {
    /**
     * repalcePatter: [{
     *    asset: "my-file.js",
     *    handler: "my-file-prefix.js"
     * }]
     */
